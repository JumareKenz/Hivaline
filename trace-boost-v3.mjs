...
top5Titles.forEach((title, i) => {
  ...
  // Test each ARV term using simple includes() for non-regex terms
  let matched = null;
  ...
  if (title === 'Dolutegravir Dosing') {
    console.log(`   DEBUG - text length: ${chunkText.length}`);
    console.log(`   DEBUG - contains 'dolutegravir': ${chunkText.includes('dolutegravir')}`);
  }
  console.log();
});
